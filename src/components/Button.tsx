...
export function Button({small=false, gray=false,className="",...props}: ButtonProps){
    ...
    return <button {...props} className={`rounded-full transition-colors duration-200 disabled:cursor-not-allowed disabled:opacity-50 text-white ${sizeClasses} ${colorClasses} ${className}`}></button>
}